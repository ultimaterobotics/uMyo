...
#include "nrf.h"
#include "leds.h"
#include "umyo_config.h"
#include "urf_radio.h"
#include "urf_star_protocol.h"
#include "urf_ble_peripheral.h"
#include "../urf_lib/urf_ble_att_process.h"
#include "../urf_lib/ble_const.h"
#include "ble_const.h"
#include "urf_timer.h"
#include "adc_read.h"
#include "fft_opt.h"
...
#define RAW_CHUNK_QUEUE_LEN 32

static uint32_t g_raw_q_tsrc[RAW_CHUNK_QUEUE_LEN];
static int16_t g_raw_q_samples[RAW_CHUNK_QUEUE_LEN][RAW_CHUNK_SAMPLES];
...
static void umyo_get_ble_mac(uint8_t *mac)
{
    uint32_t unit_id = NRF_FICR->DEVICEID[1];
    mac[0] = 212; // 0xD4
    mac[1] = (unit_id >> 24) & 0xFF;
    mac[2] = (unit_id >> 16) & 0xFF;
    mac[3] = (unit_id >> 8) & 0xFF;
    mac[4] = (unit_id) & 0xFF;
    mac[5] = 215; // 0xD7
}
...
static int umyo_build_telem_payload(uint8_t *out, int max_len)
{
    ...
    int dpos = 0;
    out[dpos++] = adc_data_id;
    out[dpos++] = battery_level;
...
static void umyo_ble_fill_services(void)
{
    ...
    umyo_service.handle = 0x30;
    umyo_service.type = UUID_PRIMARY_SERVICE;
    umyo_service.group_end = 0x3F;
    umyo_service.uuid_16 = 0;
    ble_uuid_from_text(umyo_service.uuid_128, "93375900-F229-8B49-B397-44B5899B8601");
    ble_add_service(&umyo_service);

    umyo_telem_ch.handle = 0x31;
    umyo_telem_ch.value_handle = 0x32;
    umyo_telem_ch.uuid_16 = 0;
    ble_uuid_from_text(umyo_telem_ch.uuid_128, "FC7A850D-C1A5-F61F-0DA7-9995621FBD01");

    umyo_telem_ch.val_type = VALUE_TYPE_STRUCT;
    umyo_telem_ch.val_length = 0;
    umyo_telem_ch.properties = CHARACTERISTIC_READ | CHARACTERISTIC_NOTIFY;

    umyo_telem_ch.descriptor_count = 1;
    umyo_telem_ch.descriptor_uuids[0] = 0x2902; // CCCD
    umyo_telem_ch.descriptor_handles[0] = 0x33;
    umyo_telem_ch.descriptor_values[0] = 0;
...
int prepare_and_send_BLE()
{
    ...
    if (ms - radio_last_send_time < cur_adv_delay)
        return 0;
    ...
    sBLE_flags flags;
    flags.value = 0x06; // LE General Discoverable + BR/EDR Not Supported
    data[0] = flags.value;
...
int push_adc_data()
{
    ...
    {
        // Queue the latest 8 raw EMG samples for BLE GATT raw notify mode
        {
            uint8_t idx = g_raw_q_head;

            g_raw_q_tsrc[idx] = g_raw_next_sample;
            for (int i = 0; i < RAW_CHUNK_SAMPLES; i++)
                g_raw_q_samples[idx][i] = emg_raw[i];

            g_raw_next_sample += RAW_CHUNK_SAMPLES;

            uint8_t next_head = idx + 1;
            if (next_head >= RAW_CHUNK_QUEUE_LEN)
                next_head = 0;

            if (g_raw_q_count < RAW_CHUNK_QUEUE_LEN)
            {
        ...
        for (int x = 0; x < 8; x++)
        {
            ...
            if (!mains_is_50Hz)
                fpos1 = adc_buf_pos - 9; // basic 60Hz filter
            if (fpos1 < 0)
                fpos1 += adc_buf_len;
    ...
    else
    {
        if (btn_pressed)
        {
            ...
            if (btn_time > 25) // ignore too short presses - noise
            {
                if (btn_time < 1000)
                {
                    if (ms - short_press_issued_time < 400)
                        conseq_short_presses++;
                    else
                        conseq_short_presses = 0;
...
int main(void)
{
    ...
    time_start();
    NRF_WDT->RR[0] = 0x6E524635; // reload watchdog
    leds_init(UMYO_LED_PIN_R, UMYO_LED_PIN_G, UMYO_LED_PIN_B, UMYO_LED_PIN_CORR);
    NRF_WDT->RR[0] = 0x6E524635; // reload watchdog
